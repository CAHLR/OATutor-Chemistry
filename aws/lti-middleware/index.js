...
const oatsHost = "https://quadro.ist.berkeley.edu:3001/OATutor-Chemistry/#";
...
app.post("/launch", async (req, res) => {
    ...
    if (provider.student || provider.prospective_student || provider.alumni) {
        ...
        if (!linkedLesson) {
            ...
            console.log(
                `${host}/${alreadyLinkedPage}?token=${token}&to=${linkedLesson}`
            );
            res.writeHead(302, {
...
app.post(
    ...
    async (req, res) => {
        ...
        validPut = await setLinkedLesson(
            user.resource_link_id,
            req.body.lesson.id
        );
...
app.post("/auth", async (req, res) => {
    ...
    provider.valid_request(req, (err, is_valid) => {
        if (!is_valid) {
            ...
            res.send(
                "Invalid request. Please try again or contact your teacher."
            );
            res.end();
            errFlag = true;
        }
